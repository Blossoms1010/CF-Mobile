...
final class CFCookieBridge: NSObject, WKHTTPCookieStoreObserver {
    ...
    func startObserving() {
        ...
        if observing {
            ...
            if lastObservedEphemeral != currentEphemeral {
                let prevStore: WKHTTPCookieStore = (lastObservedEphemeral ?? false) ? WebDataStoreProvider.shared.sharedEphemeralStore().httpCookieStore : WebDataStoreProvider.shared.persistentStore().httpCookieStore
                prevStore.remove(self)
                WebDataStoreProvider.shared.currentStore().httpCookieStore.add(self)
    ...
    func stopObserving() {
        ...
        let store: WKHTTPCookieStore = WebDataStoreProvider.shared.currentStore().httpCookieStore
        store.remove(self)
        observing = false
    ...
    func readCurrentCFHandleFromWK() async -> String? {
        ...
        return await withCheckedContinuation { (cc: CheckedContinuation<String?, Never>) in
            store.getAllCookies { cookies in
                ...
                // 仅当值看起来像合法的 Codeforces handle 时才返回
                if let raw = chosen?.value {
                    let t = raw.trimmingCharacters(in: .whitespacesAndNewlines)
                    let allowed = CharacterSet.alphanumerics.union(CharacterSet(charactersIn: "_-"))
                    if !t.isEmpty, t.count <= 24, t.rangeOfCharacter(from: allowed.inverted) == nil {
                        cc.resume(returning: t)
                        return
                    }
                }
                cc.resume(returning: nil)
