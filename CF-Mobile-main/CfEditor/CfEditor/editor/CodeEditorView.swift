...
struct CodeEditorView: View {
    ...
    @State private var submitCooldownUntil: Date? = nil
    @State private var showCFVerifySheet: Bool = false
    @StateObject private var cfVerifyWebModel = WebViewModel(enableProblemReader: false)
    @State private var pendingProgramTypeIdForRetry: String? = nil
    @State private var hasRetriedAfterCloudflare: Bool = false
    ...
    var body: some View {
        ...
        return base
            ...
            // Cloudflare 验证帮助页：引导用户在网页完成一次验证，随后自动重试一次提交
            .sheet(isPresented: $showCFVerifySheet) {
                NavigationStack {
                    WebView(model: cfVerifyWebModel)
                        .navigationTitle("网页验证")
                        .navigationBarTitleDisplayMode(.inline)
                        .toolbar {
                            ToolbarItem(placement: .navigationBarLeading) {
                                Button("关闭") { showCFVerifySheet = false }
                            }
                            ToolbarItem(placement: .navigationBarTrailing) {
                                Button("打开首页") { cfVerifyWebModel.load(urlString: "https://codeforces.com") }
                            }
                        }
                }
                .onAppear {
                    let back = buildSubmitPathForCurrentProblem()
                    let enter = buildEnterURL(backPath: back)
                    cfVerifyWebModel.load(urlString: enter)
                }
                .onDisappear {
                    Task {
                        await CFCookieBridge.shared.syncFromWKToHTTPCookieStorage()
                        // 自动重试一次
                        if !hasRetriedAfterCloudflare {
                            hasRetriedAfterCloudflare = true
                            if let pid = pendingProgramTypeIdForRetry {
                                pendingProgramTypeIdForRetry = nil
                                performAutoSubmit(programTypeId: pid)
                            } else {
                                performAutoSubmit()
                            }
                        }
            ...
            // 冷却计时器：10 秒提交冷却期内禁用提交
            .onReceive(Timer.publish(every: 0.5, on: .main, in: .common).autoconnect()) { _ in
                if let until = submitCooldownUntil, Date() >= until {
                    submitCooldownUntil = nil
                }
            }
    }
...
extension CodeEditorView {
    ...
    private var navigationToolbar: some ToolbarContent {
        ...
        ToolbarItemGroup(placement: .navigationBarTrailing) {
            Button {
                ...
                #endif
                if isInSubmitCooldown {
                    if let until = submitCooldownUntil {
                        let remain = Int(ceil(until.timeIntervalSinceNow))
                        presentToast("冷却中：约 \(max(remain,1))s 后可再次提交")
                    } else {
                        presentToast("冷却中，请稍后再试")
                    }
                } else {
    ...
    private var isInSubmitCooldown: Bool {
        if let until = submitCooldownUntil { return Date() < until }
        return false
    }
...
extension CodeEditorView {
    ...
    private func performAutoSubmit() {
        ...
        // 仅在成功后再进入冷却；这里先启动提交态
        hasRetriedAfterCloudflare = false
        isSubmitting = true
        Task {
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    lastSubmitAt = Date()
                    submitCooldownUntil = Date().addingTimeInterval(10)
                ...
                if !usedHandle.isEmpty {
                    ...
                    // 事后验证是否出现新的（非取消）提交，若未出现则提示失败
                    Task {
                        await verifyNewSubmissionAppeared(problem: p, baselineRunId: baselineRunId, usedHandle: usedHandle)
                    }
                }
            } catch {
                await MainActor.run {
                    // 若检测到 Cloudflare 验证需求，打开网页验证，并记录一次自动重试
                    let msg = (error as? LocalizedError)?.errorDescription ?? error.localizedDescription
                    let needsWebAuth = msg.localizedCaseInsensitiveContains("cloudflare") || msg.contains("验证") || msg.contains("未登录") || msg.localizedCaseInsensitiveContains("login") || msg.localizedCaseInsensitiveContains("enter")
                    if needsWebAuth {
                        pendingProgramTypeIdForRetry = nil
                        showCFVerifySheet = true
                        submitErrorText = "需要先完成登录/验证，请在打开的网页完成后返回。"
                    } else {
                        submitErrorText = msg
        ...
        Task {
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    let needsWebAuth = msg.localizedCaseInsensitiveContains("cloudflare") || msg.contains("验证") || msg.contains("未登录") || msg.localizedCaseInsensitiveContains("login") || msg.localizedCaseInsensitiveContains("enter")
                    if needsWebAuth {
                        pendingProgramTypeIdForRetry = programTypeId
                        showCFVerifySheet = true
                        submitErrorText = "需要先完成登录/验证，请在打开的网页完成后返回。"
                    } else {
                        submitErrorText = msg
...
extension CodeEditorView {
    // 提交后验证是否出现新提交：
    // - 若最新 runId 未变化，或只出现了 CANCELLED，视为提交失败，进行提示
    private func verifyNewSubmissionAppeared(problem p: ParsedProblem, baselineRunId: Int?, usedHandle: String) async {
        let pid = CFProblemIdentifier(contestId: p.contestId, index: p.index, name: nil)
        let deadline = Date().addingTimeInterval(16) // 最多等待 ~16s
        while Date() < deadline {
            try? await Task.sleep(nanoseconds: 1_500_000_000) // 1.5s 轮询
            do {
                if let latest = try await CFStatusAPI.latestSubmission(for: pid, handle: usedHandle) {
                    let verdict = CFVerdict.from(latest.verdict)
                    // 情况 A：有基线 runId
                    if let base = baselineRunId {
                        if latest.id != base && verdict != .cancelled {
                            return // 新提交已出现且非取消
                        }
                    } else {
                        // 情况 B：无基线，认为“最近 60s 内出现且非取消”为成功
                        let ts = TimeInterval(latest.creationTimeSeconds)
                        if verdict != .cancelled && (Date().timeIntervalSince1970 - ts) <= 60 {
                            return
                        }
                    }
                }
        ...
        // 超时仍未观察到新提交：提示失败
        await MainActor.run {
            submitErrorText = "提交失败：服务器未接收新的提交，请稍后重试。"
            showSubmitError = true
        }
    }
}
