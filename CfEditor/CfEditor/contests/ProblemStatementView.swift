                ...
                if let firstLine = inputLines.first, 
                   ...
                   testCount > 1 && testCount <= 100 {
                    ...
                    if isOutput {
                        ...
                        // 🎯 策略2：如果输出行数等于测试用例数，每行一组
                        let outputLines = lines.filter { !$0.trimmingCharacters(in: .whitespaces).isEmpty }
                        if outputLines.count == testCount {
                            ...
                            #if DEBUG
                            print("✅ 输出使用一行一组策略：\(outputLines.count) 行 = \(testCount) 个测试用例")
                            #endif
                            return result
                        }
                        
                        // 🎯 策略3：如果没有 YES/NO 模式且行数不等于测试数，使用均匀分组
                        result = []
                        let linesPerTest = max(1, outputLines.count / testCount)
                        for (index, line) in outputLines.enumerated() {
                            let groupIndex = min(index / linesPerTest, testCount - 1)
                            result.append((line, groupIndex))
                        }
                        #if DEBUG
                        print("✅ 输出使用均匀分组：每组约 \(linesPerTest) 行，测试数: \(testCount)")
                        #endif
                        return result
                    } else {
                        // === 输入侧：智能分组 ===
                        ...
                        // 🎯 策略1：尝试按空行分隔输入（跳过第一行）
                        let inputLinesWithEmpty = input.components(separatedBy: "\n")
                        ...
                        if isInputValidGrouping && !tempInputGroups.isEmpty {
                            // 使用空行分组
                            for (line, group) in zip(inputLines.dropFirst(), tempInputGroups) {
                                result.append((line, group))
                            }
                            #if DEBUG
                            print("✅ 输入使用空行分组: \(inputUniqueGroups) 组，测试数: \(testCount)")
                            #endif
                        } else {
                            // 🎯 策略2：均匀分组（回退方案）
                            ...
                            for i in 0..<remainingInputLines {
                                ...
                                result.append((inputLines[i + 1], groupIndex))
                            }
                            #if DEBUG
                            print("✅ 输入使用均匀分组: 每组约 \(linesPerTest) 行，测试数: \(testCount)")
                            #endif
                        }
