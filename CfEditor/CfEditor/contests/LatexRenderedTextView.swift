...
struct LatexRenderedTextView: View {
    ...
    var body: some View {
        LatexRenderedWebView(text: text, fontSize: fontSize, contentHeight: $contentHeight, isStable: $isStable)
...
/// 内部WebView组件
private struct LatexRenderedWebView: UIViewRepresentable {
    let text: String
    let fontSize: CGFloat
    ...
    class Coordinator: NSObject, WKScriptMessageHandler {
        var parent: LatexRenderedWebView
        var lastContentKey: String = ""
        var isLoading: Bool = false
        var lastHeightUpdate: Date = Date()
        
        init(_ parent: LatexRenderedWebView) {
            self.parent = parent
        }
