...
extension ProblemParser {
    ...
    static func extractTimeLimit(from html: String) -> String {
        if let range = html.range(of: #"<div class="time-limit">.*?(\d+\.?\d*)\s*(second|seconds|ms)"#, options: .regularExpression) {
            ...
            if let numberRange = match.range(of: #"\d+\.?\d*\s*(second|seconds|ms)"#, options: .regularExpression) {
                let fullMatch = String(match[numberRange]).trimmingCharacters(in: .whitespacesAndNewlines)
                // Convert to short format: "1.5 second" -> "1.5 s", "500 ms" -> "500 ms"
                if fullMatch.contains("second") {
                    let number = fullMatch.replacingOccurrences(of: " second", with: "")
                        .replacingOccurrences(of: " seconds", with: "")
                    return "\(number) s"
                }
                return fullMatch // Keep "ms" as is
            }
        }
        return "1 s"
    }
    ...
    static func extractMemoryLimit(from html: String) -> String {
        if let range = html.range(of: #"<div class="memory-limit">.*?(\d+)\s*megabytes"#, options: .regularExpression) {
            ...
            if let numberRange = match.range(of: #"\d+\s*megabytes"#, options: .regularExpression) {
                let fullMatch = String(match[numberRange]).trimmingCharacters(in: .whitespacesAndNewlines)
                // Convert to short format: "256 megabytes" -> "256 MB"
                let number = fullMatch.replacingOccurrences(of: " megabytes", with: "")
                    .replacingOccurrences(of: " megabyte", with: "")
                return "\(number) MB"
            }
        }
        return "256 MB"
    ...
    static func extractSamples(from html: String, isInteractive: Bool = false) -> [TestSample] {
        ...
        for (index, (input, output)) in zip(inputs, outputs).enumerated() {
            // 对于交互题，不提取分组信息
            let (cleanInput, inputGroups) = isInteractive ? (cleanHTMLPreserveNewlines(input), nil) : extractTextWithGroups(from: input)
            let (cleanOutput, outputGroups) = isInteractive ? (cleanHTMLPreserveNewlines(output), nil) : extractTextWithGroups(from: output)
