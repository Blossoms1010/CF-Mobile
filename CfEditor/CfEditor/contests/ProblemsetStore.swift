...
class ProblemsetStore: ObservableObject {
    ...
    private func loadProblems(forceRefresh: Bool) async {
        ...
        do {
            ...
            // 按contestId降序排序（最新题目在前），contestId相同的按index排序
            let sorted = result.problems.sorted { lhs, rhs in
                if let c1 = lhs.contestId, let c2 = rhs.contestId, c1 != c2 {
                    return c1 > c2  // contestId 大的在前（更新的题目）
                }
                return lhs.index < rhs.index  // 同一场比赛内按题号排序（A, B, C...）
            }
    ...
    func applyFilters() {
        ...
        if filter.minRating != nil || filter.maxRating != nil {
            ...
            if maxRating < minRating {
                ...
                result = result.filter { problem in
                    // 如果有评分，检查是否在范围内
                    if let rating = problem.rating, rating > 0 {
                        return rating >= minRating && rating <= maxRating
                    }
                    // 没有评分的题目（undefined）也保留，包括：
                    // - Gym 题目 (contestId >= 100000)
                    // - 愚人节比赛
                    // - 刚结束还没评分的比赛
                    // - 其他特殊类型比赛
                    return true
