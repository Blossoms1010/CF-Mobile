...
class ProblemCache: ObservableObject {
    ...
    /// 获取题目（优先从缓存，如果没有则下载）
    func getProblem(contestId: Int, problemIndex: String, tags: [String]? = nil, forceRefresh: Bool = false) async throws -> ProblemStatement {
        ...
        // 下载新数据
        let statement = try await ProblemParser.fetchAndParse(contestId: contestId, problemIndex: problemIndex, tags: tags)
