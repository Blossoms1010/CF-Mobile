...
private struct ProblemWebPage: View {
    ...
    @State private var availableModels: [AITranslationModel] = []
    ...
    var body: some View {
        ...
        .alert("翻译失败", isPresented: .constant(translationError != nil)) {
            Button("确定") {
            ...
            Button(action: {
                ...
                if isTranslating {
                    ...
                    Label("翻译", systemImage: "character.book.closed")
                        .foregroundColor(isAITranslatorConfigured ? .primary : .gray)
                } else {
    ...
    private var isAITranslatorConfigured: Bool {
        ...
        // 兼容旧配置
        return !aiTransModel.trimmed.isEmpty && !aiTransProxyApi.trimmed.isEmpty
    }
    
    private func getSelectedModel() -> AITranslationModel? {
    ...
    private func loadAndMigrateModels() {
        // 尝试加载新配置
        if !modelsData.isEmpty, let models = try? JSONDecoder().decode([AITranslationModel].self, from: modelsData) {
        ...
        // 如果新配置为空，尝试从旧配置迁移
        else if !aiTransModel.trimmed.isEmpty && !aiTransProxyApi.trimmed.isEmpty {
            let legacyModel = AITranslationModel(
                name: aiTransModelName.trimmed.isEmpty ? "Legacy Model" : aiTransModelName.trimmed,
                model: aiTransModel.trimmed,
                apiEndpoint: aiTransProxyApi.trimmed,
                apiKey: aiTransApiKey.trimmed
            )
            availableModels = [legacyModel]
                ...
                if availableModels.count > 1 {
                    ToolbarItem(placement: .principal) {
                        Menu {
                            ...
                            HStack(spacing: 4) {
                                Text(getSelectedModel()?.name ?? "Select Model")
                                    .font(.subheadline)
                                    .fontWeight(.medium)
        ...
        web.collectSectionBasedSegments { sectionData in
            Task {
                ...
                if let selectedModel = getSelectedModel() {
                    ...
                    proxyAPI = selectedModel.apiEndpoint
                    apiKey = selectedModel.apiKey.isEmpty ? nil : selectedModel.apiKey
                } else {
                    // 使用旧配置作为后备
                    modelId = aiTransModel.trimmed
                    proxyAPI = aiTransProxyApi.trimmed
                    apiKey = aiTransApiKey.trimmed.isEmpty ? nil : aiTransApiKey.trimmed
                }
                ...
                for section in sectionOrder {
                    ...
                    // 在主线程上初始化这个section，显示空的翻译
                    await MainActor.run {
                        let emptySegments = segments.map { TranslationSegment(original: $0, translated: "") }
                        self.sectionTranslations.append(SectionTranslation(section: section, segments: emptySegments))
                    }
                    
                    // 获取当前section在数组中的索引（刚刚添加的）
                    let currentSectionIndex = await MainActor.run { 
                        self.sectionTranslations.count - 1
                    }
                    ...
                    do {
                        ...
                        try await AITranslator.translateENtoZHStreaming(
                            ...
                            await MainActor.run {
                                // 更新对应位置的翻译内容
                                var currentSegments = self.sectionTranslations[currentSectionIndex].segments
                                currentSegments[index] = TranslationSegment(
                                    original: segments[index],
                                    translated: translated
                                )
                                self.sectionTranslations[currentSectionIndex] = SectionTranslation(
                                    section: section,
                        ...
                        let finalSegments = await MainActor.run { 
                            self.sectionTranslations[currentSectionIndex].segments
                        }
                        resultSections.append(SectionTranslation(section: section, segments: finalSegments))
                        ...
                        if !hasError {
                            hasError = true
                            let errorMsg: String
                            if let transError = error as? AITranslator.TranslationError {
                                errorMsg = transError.message
                            } else {
                                errorMsg = error.localizedDescription
                            }
                            
                            await MainActor.run {
                                self.translationError = "翻译失败：\(errorMsg)\n\n请检查：\n1. API 端点是否正确\n2. API Key 是否有效\n3. 模型名称是否正确\n4. 网络连接是否正常"
                                self.isTranslating = false
                            }
                            return
                        }
                ...
                await MainActor.run {
                    if resultSections.isEmpty {
                        self.translationError = "未找到可翻译的内容"
                    }
                    self.isTranslating = false
                }
            }
