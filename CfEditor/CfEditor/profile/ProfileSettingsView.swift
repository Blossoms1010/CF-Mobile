...
struct ProfileSettingsView: View {
    ...
    @StateObject private var judge0ConfigManager = Judge0ConfigManager.shared
    ...
    var body: some View {
        Form {
            ...
            Section {
                Picker("API 类型", selection: $judge0ConfigManager.configuration.apiType) {
                    ForEach(Judge0APIType.allCases) { apiType in
                        HStack {
                ...
                // 显示当前 API 类型的描述
                Text(judge0ConfigManager.configuration.apiType.description)
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .padding(.vertical, 4)
                
                // RapidAPI 配置
                if judge0ConfigManager.configuration.apiType == .rapidapi {
                    VStack(alignment: .leading, spacing: 8) {
                        SecureField("X-RapidAPI-Key", text: $judge0ConfigManager.configuration.rapidAPIKey)
                            .textInputAutocapitalization(.never)
                            .autocorrectionDisabled(true)
                        
                        Text("从 RapidAPI 获取您的 API Key")
                            .font(.caption2)
                ...
                // 自定义 API 配置
                if judge0ConfigManager.configuration.apiType == .custom {
                    VStack(alignment: .leading, spacing: 8) {
                        TextField("API URL", text: $judge0ConfigManager.configuration.customURL)
                            .textInputAutocapitalization(.never)
                            .autocorrectionDisabled(true)
                            .keyboardType(.URL)
                        
                        Text("示例: https://your-judge0-server.com")
                            .font(.caption2)
                            .foregroundStyle(.tertiary)
                    }
                    
                    VStack(alignment: .leading, spacing: 8) {
                        SecureField("API Key (可选)", text: $judge0ConfigManager.configuration.customAPIKey)
                            .textInputAutocapitalization(.never)
                            .autocorrectionDisabled(true)
                ...
                HStack {
                    Text("当前 API:")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Text(judge0ConfigManager.configuration.effectiveURL)
                ...
                Label("Judge0 代码执行", systemImage: "play.circle.fill")
                    .font(.headline)
            } footer: {
                Text("配置用于运行代码的 Judge0 API。社区版 API 免费但有速率限制，建议自建实例以获得更好体验。")
                    .font(.footnote)
            }
