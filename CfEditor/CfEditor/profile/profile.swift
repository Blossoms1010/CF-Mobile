...
struct HeatmapData {
    ...
    private static func calculateYearView(from submissions: [CFSubmission], forYear year: Int) -> HeatmapData {
        ...
        let ratedOK = submissions.filter { 
            $0.verdict == "OK" && $0.problem.rating != nil 
        }
        ...
        return HeatmapData(weeks: weeks, dailyColors: dailyColors, monthLabels: monthLabels, viewType: .year(year))
    }
    ...
    private static func calculate365DaysView(from submissions: [CFSubmission]) -> HeatmapData {
        ...
        return HeatmapData(weeks: weeks, dailyColors: dailyColors, monthLabels: monthLabels, viewType: .rolling365)
    }
}
...
struct ProfileView: View {
    ...
    private var profileDetails: some View {
        Form {
            ...
            Section {
                ...
                Label("Info", systemImage: "chart.bar.xaxis")
            }

            // Rating 曲线：Skeleton + 淡入
            Section { ratingChartBox } header: {
                Label("Rating graph", systemImage: "chart.line.uptrend.xyaxis")
            }
            
            // 热力图：Skeleton + 淡入
            Section { heatmapBox } header: {
                Label("Rating-Based Heatmap", systemImage: "grid")
            }
            
            // 练习柱状图：Skeleton + 淡入 + 数据变化动画
            Section { practiceHistogramBox } header: {
                Label("Rating Solved", systemImage: "chart.bar")
            }
            
            // 标签分布：Skeleton + 淡入 + 数据变化动画
            Section { tagPieBox } header: {
                Label("Tag Solved", systemImage: "chart.pie")
            }
            ...
            Section {
                ...
                Label("Recent Submissions", systemImage: "clock")
            } footer: {
    ...
    private var heatmapBox: some View {
        if let heatmapData {
            ...
            SkeletonChartBlock(height: 150) // 稍微增加高度来容纳年份选择器
        }
    }
            ...
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(problemTitle(s))
                        ...
                        .truncationMode(.middle)
                    HStack(spacing: 6) {
                        Text(CFVerdict.from(s.verdict).englishText)
                            .font(.caption)
                            .foregroundStyle(.secondary)
    ...
    private var ratingTiers: [(name: String, range: Range<Int>, color: Color)] {
        [
            ...
            ("International Grandmaster", 2600..<3000, darkRed), ("Legendary Grandmaster", 3000..<5000, deepRed)
        ]
    }
}
                ...
                VStack(spacing: 4) {
                    HStack(spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            HStack(spacing: 6) {
                                Text(CFVerdict.from(s.verdict).englishText).font(.caption).foregroundStyle(.secondary)
                                if let lang = s.programmingLanguage, !lang.isEmpty {
