        ...
        (function(){
          try{
            ...
            for (const el of candidates){
              ...
              // 检查是否是新的部分标题 - 只有Input以后的部分才分离，Input之前的所有内容都归到Legend
              ...
              if (lowerText === 'input' || lowerText.includes('input specification')) {
                ...
                continue;
              } else if ((lowerText === 'note' || lowerText === 'notes' || lowerText.includes('note')) && 
                        (currentSection === 'Output' || currentSection === 'Input')) {
                // 只有在Input/Output部分之后才识别Note为独立部分
                currentSection = 'Note';
                continue;
              ...
              // 检查元素的父容器类来确定部分 - 只在Input后的部分生效
              if (el.closest('.input-specification')) {
                currentSection = 'Input';
              } else if (el.closest('.output-specification')) {
                currentSection = 'Output';
              } else if ((el.closest('.note') || el.closest('.notes')) && 
                        (currentSection === 'Output' || currentSection === 'Input' || currentSection === 'Note')) {
                // 只有在Input/Output之后才将note容器识别为Note部分
                currentSection = 'Note';
          ...
          const preToText = (pre) => {
            ...
            const serialize = (node) => {
              ...
              if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                // 跳过注入的复制按钮及提示元素（新旧两套样式都要过滤）
                if (node.classList && (
                  node.classList.contains('cf-copy-wrap') || 
                  node.classList.contains('cf-copied-tip') ||
                  node.classList.contains('cf-copy-title-btn') ||
                  node.classList.contains('cf-mini-tip')
                )) { return; }
                if (tag === 'BUTTON' && node.classList && (
                  node.classList.contains('cf-copy-btn') ||
                  node.classList.contains('cf-copy-title-btn')
                )) { return; }
                if (tag === 'BR') { text += '\n'; return; }
...
struct WebView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        if model.enableProblemReader {
            ...
            (function(){try{
              ...
              // 检查是否已注入
              if (document.getElementById('cf-early-styles')) return;
              
              // 创建样式元素
              var style = document.createElement('style');
              style.id = 'cf-early-styles';
              style.textContent = `
                :root{ 
                  --cf-bg: #ffffff; 
                  --cf-text: #2c2c2e; 
                ...
                .sample-tests > .sample-test:hover {
                ...
                /* 样例高亮状态 - 使用更高的权重和边框使其更明显 */
                .sample-tests .input.cf-highlighted,
                .sample-tests .output.cf-highlighted {
                ...
                @media (prefers-color-scheme: dark) {
                  ...
                  .sample-tests .output.cf-highlighted {
                    ...
                    box-shadow: 0 0 0 2px rgba(255, 193, 7, 0.08) !important;
                  }
                }
                ...
                .sample-tests .input { border-bottom: 1px solid var(--cf-border); }
                
                .sample-tests pre { 
                  margin: 0; 
                  padding: 14px 16px;
                  background: var(--cf-sample-bg) !important;
                ...
                /* 自定义滚动条 */
                .sample-tests pre::-webkit-scrollbar {
                  height: 8px;
                }
                
                .sample-tests pre::-webkit-scrollbar-track {
                  background: transparent;
                }
                
                .sample-tests pre::-webkit-scrollbar-thumb {
                  background: var(--cf-border);
                  border-radius: 4px;
                }
                
                .sample-tests pre::-webkit-scrollbar-thumb:hover {
                  background: var(--cf-text-secondary);
                }
                
                /* 选择高亮 */
                .sample-tests pre::selection {
                ...
                .input:hover .cf-copy-title-btn,
                .output:hover .cf-copy-title-btn,
                .cf-copy-title-btn:focus {
                  opacity: 1;
                  pointer-events: auto;
                }
                
                .cf-copy-title-btn:hover {
                ...
                .cf-copy-title-btn:active {
                  transform: scale(0.96) translateY(0);
                  background: var(--cf-border);
                }
                
                .cf-copy-title-btn.copied {
                  background: #10b981;
                  border-color: #10b981;
                  color: white;
                  opacity: 1;
                ...
                .cf-copy-title-btn.copied:hover {
                  background: #059669;
                  border-color: #059669;
                  transform: translateY(-1px);
                }
                ...
                .cf-copy-title-btn:focus-visible {
                  outline: 2px solid var(--cf-link-color);
                  outline-offset: 2px;
                  opacity: 1;
                  pointer-events: auto;
                }
                ...
                @media (max-width: 768px) {
                  .sample-tests > .sample-test { margin-bottom: 16px; border-radius: 8px; }
                  .sample-tests pre { font-size: 13px; padding: 12px; }
                  .cf-copy-title-btn { opacity: 1; pointer-events: auto; }
                }
                
                @media (max-width: 480px) {
                  .sample-tests > .sample-test { margin-bottom: 12px; }
                  .sample-tests pre { font-size: 12.5px; padding: 10px; }
                  .sample-tests .input > .title,
                  .sample-tests .output > .title { font-size: 13px; padding: 8px 12px; }
                }
                
                /* 打印优化 */
                @media print {
                  .cf-copy-title-btn,
                  .cf-mini-tip { display: none !important; }
                  .sample-tests > .sample-test {
              ...
              // 尽早注入到 head 或 documentElement
              (document.head || document.documentElement).appendChild(style);
            }catch(e){}})();
            """#
            let styleScript = WKUserScript(source: styleJS, injectionTime: .atDocumentStart, forMainFrameOnly: true)
            config.userContentController.addUserScript(styleScript)
        }

        // 后面做 CodeMirror 或注入脚本也在这里加
        let web = WKWebView(frame: .zero, configuration: config)
        // 题面生成回调 handler（用于一键生成文件）
        config.userContentController.add(context.coordinator, name: "cfReader")
    ...
    static func dismantleUIView(_ uiView: WKWebView, coordinator: Coordinator) {
        ...
        uiView.configuration.userContentController.removeScriptMessageHandler(forName: "cfReader")
    ...
    static func desktopSafariUserAgent() -> String {
        // macOS 14 Safari 17 UA
        return "Mozilla/5.0 (Macintosh; Intel Mac OS X 14_0) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Safari/605.1.15"
    }

    /// Apply the preferred UA mode stored in UserDefaults to the given WKWebView.
    /// Modes: "mobile" | "desktop" | "system" (default)
    static func applyPreferredUserAgent(to webView: WKWebView) {
        let mode = UserDefaults.standard.string(forKey: "web.uaMode") ?? "system"
        switch mode {
        case "mobile":
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
        ...
        private func injectCodeforcesProblemReader(on webView: WKWebView) {
            // 先检查 DOM 是否准备就绪
            checkDOMReadyAndInject(on: webView, attemptCount: 0)
        }
        
        // 递归检查 DOM 准备状态，无限重试直到成功
        private func checkDOMReadyAndInject(on webView: WKWebView, attemptCount: Int) {
            let checkJS = #"""
            (function(){
              try {
                // 检查页面是否已加载且题面元素存在
                ...
                const hasContent = problem && (problem.textContent || '').trim().length > 100;
                return { ready: ready, hasProblem: !!problem, hasContent: hasContent };
              } catch(e) {
                return { ready: false, hasProblem: false, hasContent: false, error: e.message };
              }
            })();
            """#
            
            webView.evaluateJavaScript(checkJS) { [weak self, weak webView] result, error in
                guard let self = self, let webView = webView else { return }
                
                if let dict = result as? [String: Any],
                   let hasContent = dict["hasContent"] as? Bool,
                   hasContent {
                    // DOM 准备就绪，执行注入
                    print("✅ 题面提取成功（尝试 \(attemptCount + 1) 次）")
                    self.performProblemReaderInjection(on: webView)
                } else {
                    // 继续重试，每次间隔 300ms
                    // 每 10 次打印一次进度日志
                    if attemptCount > 0 && attemptCount % 10 == 0 {
                        print("⏳ 等待题面加载... (已尝试 \(attemptCount) 次)")
                    }
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                        self.checkDOMReadyAndInject(on: webView, attemptCount: attemptCount + 1)
                    }
                }
            }
        }
        ...
        // 执行实际的题面提取和渲染
        private func performProblemReaderInjection(on webView: WKWebView) {
            ...
            (function(){
              try {
                ...
                if (!problem) { 
                  ...
                  removeOverlay(); 
                  return { success: false, message: 'Problem element not found' };
                }
                ...
                (function(){
                  try{
                    var btn = document.getElementById('cf-generate-file-btn');
                    if(!btn){
                      btn = document.createElement('button');
                      btn.id = 'cf-generate-file-btn';
                      btn.textContent = '一键生成文件';
                      btn.style.cssText = 'position:fixed;top:calc(env(safe-area-inset-top) + 10px);right:calc(env(safe-area-inset-right) + 10px);z-index:2147483647;padding:8px 12px;font-size:13px;border-radius:8px;border:1px solid var(--cf-border);background:var(--cf-bg);color:var(--cf-text);box-shadow:0 2px 8px rgba(0,0,0,0.12);cursor:pointer;';
                      btn.onmouseenter=function(){ btn.style.boxShadow='0 4px 12px rgba(0,0,0,0.16)'; };
                      btn.onmouseleave=function(){ btn.style.boxShadow='0 2px 8px rgba(0,0,0,0.12)'; };
                      document.body.appendChild(btn);

                      var parseCF = function(){
                        var path = (location.pathname||'').toLowerCase();
                        var contestId = null, index = null;
                        var m1 = path.match(/\\/problemset\\/problem\\/(\\d+)\\/([a-z]\\d?)/);
                        if(m1){ contestId = parseInt(m1[1],10); index = m1[2].toUpperCase(); }
                        var m2 = path.match(/\\/contest\\/(\\d+)\\/problem\\/([a-z]\\d?)/);
                        if(!contestId && m2){ contestId = parseInt(m2[1],10); index = m2[2].toUpperCase(); }
                        if(!contestId){
                          var title = (document.querySelector('.problem-statement .header .title')||{}).innerText||'';
                          var m3 = title.match(/(\\d+)([A-Za-z]\\d?)/);
                          if(m3){ contestId = parseInt(m3[1],10); index = m3[2].toUpperCase(); }
                        }
                      ...
                      btn.addEventListener('click', function(e){
                        ...
                        try{
                          var info = parseCF();
                          var fn = (info.contestId ? (String(info.contestId) + (info.index||'A')) : 'solution') + '.cpp';
                          if(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.cfReader){
                            window.webkit.messageHandlers.cfReader.postMessage({ action: 'generate', contestId: info.contestId||0, index: info.index||'A', fileName: fn });
                          }
                        }catch(err){}
                      });
                    }
                ...
                style.textContent = `
                  ...
                  /* Input 和 Output 之间的分隔线 */
                  .sample-tests .input + .output::before {
                    content: '';
                    display: block;
                    height: 1px;
                    background: var(--cf-border);
                  ...
                  /* 深色模式下的公式适配 - 使用 currentColor 让公式继承文字颜色 */
                  @media (prefers-color-scheme: dark) {
                    mjx-container svg, mjx-container path, mjx-container rect, mjx-container text {
                      fill: currentColor !important;
                      stroke: currentColor !important;
                    }
                    /* 确保 MathJax 生成的元素使用正确的颜色 */
                    mjx-math, mjx-mi, mjx-mn, mjx-mo, mjx-mtext {
                      color: var(--cf-text) !important;
                    }
                  }
                  ...
                  /* 渲染前的公式隐藏（避免显示原始 LaTeX 代码） */
                  .problem-statement script[type*="math/tex"] {
                    display: none;
                  }
                  
                  /* 公式容器淡入动画，减少突兀感 */
                  mjx-container {
                    animation: mjxFadeIn 0.2s ease-in;
                  }
                  @keyframes mjxFadeIn {
                    from { opacity: 0; }
                    to { opacity: 1; }
                  ...
                  /* Input/Output 标题区域 - 仿 Codeforces 官方设计 */
                  .sample-tests .input > .title,
                  .sample-tests .output > .title {
                    display: flex;
                    align-items: center;
                  ...
                  /* Input 区域的边框（顶部圆角） */
                  .sample-tests .input > .title:first-child {
                    border-top-left-radius: 8px;
                    border-top-right-radius: 8px;
                  }
                  
                  /* Output 区域在 Input 之后时的特殊处理 */
                  .sample-tests .input + .output > .title {
                    border-top: 1px solid var(--cf-border);
                  }
                  
                  /* 最后一个 Output 的底部圆角 */
                  .sample-tests .output:last-child > pre {
                  ...
                  /* 鼠标悬停在 Input/Output 容器上时显示 Copy 按钮 */
                  .input:hover .cf-copy-title-btn,
                  .output:hover .cf-copy-title-btn,
                  .cf-copy-title-btn:focus {
                    opacity: 1;
                ...
                // 强制修复夜间模式下输入输出区域的文字颜色
                function forceDarkModeTextColor() {
                  ...
                  selectors.forEach(function(selector) {
                    ...
                    elements.forEach(function(element) {
                      if (isDarkMode) {
                        // 夜间模式：深灰色背景 + 浅色文字，提供对比度
                        element.style.setProperty('background', '#1a1a1a', 'important');
                  ...
                  titleSelectors.forEach(function(selector) {
                    ...
                    elements.forEach(function(element) {
                      if (isDarkMode) {
                        ...
                        element.style.setProperty('background', '#1a1a1a', 'important');
                        element.style.setProperty('color', '#e0e0e0', 'important');
                        element.style.setProperty('border-bottom', '1px solid #3a3a3c', 'important');
                      } else {
                        // 日间模式：清除强制样式
                        element.style.removeProperty('background');
                        element.style.removeProperty('color');
                  ...
                  if (isDarkMode) {
                    ...
                    allElements.forEach(function(el) {
                      ...
                      if (bgColor === 'rgb(255, 255, 255)' || 
                          ...
                          bgColor === '#ffffff' ||
                          bgColor === '#fff' ||
                          bgColor.match(/rgb\(25[0-5], 25[0-5], 25[0-5]\)/)) {
                        // 使用深灰色而不是纯黑色，保持一些对比度
                    ...
                    allElements.forEach(function(el) {
                      // 移除可能被强制设置的暗色样式
                      if (el.style.backgroundColor === '#000000' || 
                ...
                // 立即执行一次
                setTimeout(forceDarkModeTextColor, 100);
                
                // 监听DOM变化
                var observer = new MutationObserver(function() {
                  setTimeout(forceDarkModeTextColor, 50);
                });
                ...
                if (window.matchMedia) {
                  window.matchMedia('(prefers-color-scheme: dark)').addListener(forceDarkModeTextColor);
                }
                ...
                if (sampleRoot) {
                  ...
                  // 添加样例点击高亮功能
                  try {
                    const sampleTests = sampleRoot.querySelectorAll('.sample-test');
                    sampleTests.forEach(test => {
                      const input = test.querySelector('.input');
                      const output = test.querySelector('.output');
                      if (!input || !output) return;
                      
                      // 点击输入或输出区域高亮整组，再次点击同一组取消高亮
                      const handleClick = (e) => {
                        // 如果点击的是复制按钮，不处理高亮
                        if (e.target.closest('.cf-copy-title-btn')) return;
                        
                        const isCurrentlyHighlighted = input.classList.contains('cf-highlighted');
                        
                        // 先移除所有其他样例的高亮
                        document.querySelectorAll('.sample-tests .cf-highlighted').forEach(el => {
                          el.classList.remove('cf-highlighted');
                        });
                        
                        // 如果当前组没有高亮，则高亮它；如果已经高亮，则保持取消状态
                        if (!isCurrentlyHighlighted) {
                          input.classList.add('cf-highlighted');
                          output.classList.add('cf-highlighted');
                        }
                    ...
                    console.error('Error setting up sample highlight:', e);
                  }
                }
                  ...
                  s.onload = function(){ 
                    try { 
                      if (window.MathJax && window.MathJax.typesetPromise) {
                        window.MathJax.typesetPromise([container]).catch(function(err){
                          console.warn('Initial MathJax typeset error:', err);
                        });
                      }
                    } catch(e){
                      console.error('MathJax onload error:', e);
                    } 
                  };
                ...
                // 智能 MathJax 渲染时机控制（避免闪烁和重复渲染）
                (function smartMathJaxRender() {
                  let renderAttempts = 0;
                  const maxAttempts = 3;
                  let lastRenderTime = 0;
                  const minRenderInterval = 500; // 最小渲染间隔，避免频繁重渲染
                  ...
                  function attemptRender() {
                    if (renderAttempts >= maxAttempts) {
                      console.log('MathJax render attempts exhausted');
                      return;
                    }
                    
                    if (!window.MathJax || !window.MathJax.typesetPromise) {
                      renderAttempts++;
                      setTimeout(attemptRender, 300 * renderAttempts); // 递增延迟
                      return;
                    }
                    ...
                    try {
                      window.MathJax.typesetPromise([container]).then(function() {
                        console.log('MathJax delayed render completed');
                      }).catch(function(err) {
                        console.warn('MathJax delayed render error:', err);
                        if (renderAttempts < maxAttempts) {
                          renderAttempts++;
                      ...
                      console.error('MathJax render exception:', e);
                    }
                  }
                  ...
                  if (window.matchMedia) {
                    window.matchMedia('(prefers-color-scheme: dark)').addListener(function() {
                      setTimeout(function() {
                        if (shouldRender() && window.MathJax && window.MathJax.typesetPromise) {
                          try {
                ...
                // 成功完成，移除加载遮罩
                removeOverlay();
                return { success: true, message: 'Injection completed successfully' };
              } catch (e) { 
                // 如果出现任何错误，确保移除遮罩
            ...
            webView.evaluateJavaScript(js) { [weak self, weak webView] result, error in
                guard let self = self, let webView = webView else { return }
                ...
                if let dict = result as? [String: Any],
                   let success = dict["success"] as? Bool {
                    if !success {
                        ...
                        print("题面提取失败，将重试: \(message)")
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                            self.checkDOMReadyAndInject(on: webView, attemptCount: 0)
                        }
                    }
                } else if let error = error {
                    print("题面注入 JavaScript 错误，将重试: \(error.localizedDescription)")
        ...
        // 移除加载遮罩，可选择是否显示错误提示
        private func removeProblemReaderOverlay(on webView: WKWebView, showError: Bool = false) {
            let js: String
            if showError {
                js = #"""
                (function(){
                  try {
                    var l=document.getElementById('cf-reader-loading');
                    if(l && l.parentNode) {
                      // 保持遮罩并提示重试，不移除以避免露出原生页面
                      l.innerHTML = '\
                        <div style="text-align:center;line-height:1.7">\
                          <div class="ring" style="margin:0 auto 10px"></div>\
                          <div>仍在提取题面… 正在重试</div>\
        ...
        // MARK: - WKScriptMessageHandler for cfReader
        func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
            guard message.name == "cfReader" else { return }
            guard let body = message.body as? [String: Any] else { return }
            let action = (body["action"] as? String) ?? ""
            if action == "generate" {
                let contestId = (body["contestId"] as? Int) ?? 0
                let index = (body["index"] as? String) ?? "A"
                let fileName = (body["fileName"] as? String) ?? "solution.cpp"
                handleGenerateFile(contestId: contestId, index: index, fileName: fileName)
            }
        }

        private func handleGenerateFile(contestId: Int, index: String, fileName: String) {
            model.extractCodeforcesSamples { pairs in
