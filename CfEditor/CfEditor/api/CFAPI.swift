...
enum CFVerdict: String {
    ...
    static func from(_ raw: String?) -> CFVerdict {
        guard let s = raw else { return .unknown }
        return CFVerdict(rawValue: s) ?? .unknown
    }
...
enum CFStatusAPI {
    ...
    static func submissionsFor(problem: CFProblemIdentifier, handle: String, limit: Int = 1000) async throws -> [CFSubmission] {
        ...
        if let subs = try? await contestStatus(contestId: problem.contestId, handle: handle, from: 1, count: limit) {
            ...
            if !filtered.isEmpty { return filtered.sorted(by: { $0.creationTimeSeconds > $1.creationTimeSeconds }) }
        }
        ...
        // 退路 B：无题目名时，按 index 近似匹配（大小写不敏感）
        let subs = try await userStatus(handle: handle, from: 1, count: limit)
        let matchedByIndex = subs.filter { s in s.problem.index.caseInsensitiveCompare(problem.index) == .orderedSame }
        return matchedByIndex.sorted(by: { $0.creationTimeSeconds > $1.creationTimeSeconds })
    }
