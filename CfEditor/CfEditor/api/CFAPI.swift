...
enum CFVerdict: String {
    ...
    var englishText: String {
        switch self {
        case .ok: return "AC"
        case .wrongAnswer: return "WA"
        case .timeLimit: return "TL"
        case .memoryLimit: return "ML"
        case .runtimeError: return "RE"
        case .compilationError: return "CE"
        case .presentationError: return "PE"
    ...
    // 获取带测试点信息的显示文本
    func textWithTestInfo(passedTests: Int?) -> String {
        let base = self.englishText
        
        // 对于 TL, ML, WA, RE 这些需要显示测试点的情况
        switch self {
        case .timeLimit, .memoryLimit, .wrongAnswer, .runtimeError:
            if let passed = passedTests {
                return "\(base) test\(passed + 1)"
            }
            return base
        default:
            return base
