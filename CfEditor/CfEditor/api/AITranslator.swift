...
struct AITranslator {
    ...
    /// 逐段翻译并通过回调实时返回结果
    static func translateENtoZHStreaming(
        _ texts: [String], 
        model: String, 
        proxyAPI: String, 
        apiKey: String?,
        onSegmentTranslated: @escaping (Int, String) async throws -> Void
    ) async throws {
        ...
        for (index, text) in texts.enumerated() {
            ...
            if trimmed.isEmpty {
                try await onSegmentTranslated(index, text)
                continue
            }
            do {
                ...
                // 立即通过回调返回翻译结果
                try await onSegmentTranslated(index, zh)
            } catch {
