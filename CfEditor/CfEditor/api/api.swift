...
struct CFRanklistRow: Decodable, Sendable {
    let party: CFParty
    let rank: Int?
}

struct CFParty: Decodable, Sendable {
    let members: [CFMember]
    let participantType: String
}

struct CFMember: Decodable, Sendable {
    let handle: String
}
...
actor CFAPI {
    ...
    private let session: URLSession = {
        ...
        cfg.timeoutIntervalForRequest = 30  // 增加单个请求超时时间
        cfg.timeoutIntervalForResource = 60  // 增加资源超时时间
    ...
    func userAllSubmissions(handle: String, forceRefresh: Bool = false) async throws -> [CFSubmission] {
        // Codeforces API 单次请求最多返回 10000 条记录
        // 对于提交数超过 10000 的用户（如 jiangly），需要分批获取
        var allSubmissions: [CFSubmission] = []
        var from = 1
        let pageSize = 10000 // 使用最大限制以减少请求次数
        let maxPages = 20 // 最多获取 20 页（200,000 条记录），防止无限循环
        var pageCount = 0
        
        while pageCount < maxPages {
            pageCount += 1
            
            let batch: [CFSubmission] = try await get(method: "user.status", params: [
                "handle": handle,
                "from": String(from),
                "count": String(pageSize)
            ], forceRefresh: forceRefresh && from == 1) // 只在第一次请求时使用 forceRefresh
            
            if batch.isEmpty {
                break
            }
            
            allSubmissions.append(contentsOf: batch)
            
            // 如果返回的数量少于请求的数量，说明已经到底了
    ...
    // 获取用户博客条目数量
    func userBlogEntryCount(handle: String) async throws -> Int {
        struct BlogEntry: Decodable {
            let id: Int
        }
        let entries: [BlogEntry] = try await get(method: "user.blogEntries", params: [
            "handle": handle
        ])
        ...
        let task = Task<Data, Error> {
            ...
            while true {
                ...
                do {
                    ...
                    var req = URLRequest(url: url, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: 30)  // 增加单个请求超时
                    req.httpMethod = "GET"
    ...
    private func shouldUseDiskCache(method: String) -> Bool {
        switch method {
        case "contest.list", "contest.standings", "user.rating", "problemset.problems", "user.info":
            return true // 历史数据和用户基本信息，很少变化，适合磁盘缓存以提高稳定性
        default:
    ...
    private func defaultTTL(for method: String, params: [String: String]) -> TimeInterval? {
        ...
        case "user.info":
            return 7200 // 2 小时（用户信息变化不频繁，增加缓存时间提高稳定性）
        case "user.status":
            // 根据请求数量和用途优化缓存时间
            if let countStr = params["count"], let count = Int(countStr) {
                return count > 1000 ? 3600 : 600 // 大量历史数据缓存更久（提交记录）
            }
            return 600 // 默认 10 分钟（提高稳定性）
        case "user.rating":
            return 31_536_000 // 365 天（rating 历史几乎不变）
        case "user.blogEntry.view":
            return 7200 // 2 小时（博客数量变化不频繁）
        case "problemset.problems":
            return 86_400 // 1 天
        case "problemset.recentStatus":
            return 180 // 3 分钟
        default:
            return 900 // 默认 15 分钟（更积极的缓存）
        }
    }
