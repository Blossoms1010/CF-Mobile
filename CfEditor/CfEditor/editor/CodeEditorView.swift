...
struct CodeEditorView: View {
    ...
    private var hasCFLogin: Bool {
        ...
        return cookies.contains { $0.name == "X-User" && $0.domain.lowercased().contains("codeforces.com") }
    }
    ...
    var body: some View {
        ...
        return base
            ...
            .sheet(isPresented: $isSubmitPresented) {
                SafariView(url: URL(string: buildSubmitURLForCurrentProblem())!)
            }
            // 右上角动作面板（上下滑动的抽屉）
        ...
        ToolbarItemGroup(placement: .navigationBarTrailing) {
            Button {
                ...
                if hasCFLogin {
                    ...
                    presentToast("请先登录 Codeforces（获取 Cookie）后提交")
                }
            } label: {
                Image(systemName: "paperplane.fill")
                    .foregroundStyle(hasCFLogin ? .primary : .secondary)
                    .opacity(hasCFLogin ? 1 : 0.5)
            }
