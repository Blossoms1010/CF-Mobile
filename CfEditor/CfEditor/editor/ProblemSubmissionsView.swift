...
struct ProblemSubmissionsView: View {
    ...
    private var emptyView: some View {
        VStack(spacing: 16) {
            ...
            if handle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                Text("No public submission")
                    .font(.footnote)
                    .foregroundColor(.secondary)
                    .multilineTextAlignment(.center)
            } else {
                Text("\(handle) did not submit this problem")
                    .font(.footnote)
                    .foregroundColor(.secondary)
                    .multilineTextAlignment(.center)
            }
    ...
    private func verdictDisplayText(_ verdict: String?, passedTests: Int?) -> String {
        ...
        switch verdict {
        case "OK": baseText = "AC"
        case "WRONG_ANSWER": baseText = "WA"
        case "TIME_LIMIT_EXCEEDED": baseText = "TL"
        case "MEMORY_LIMIT_EXCEEDED": baseText = "ML"
        case "RUNTIME_ERROR": baseText = "RE"
        case "COMPILATION_ERROR": baseText = "CE"
