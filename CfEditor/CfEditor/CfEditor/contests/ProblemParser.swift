...
class ProblemParser {
    ...
    private static func extractSamples(from html: String) -> [TestSample] {
        ...
        for (i, input) in inputs.enumerated() {
            print("🔍 输入 #\(i+1) (前100字符): \(input.prefix(100))")
        }
        for (i, output) in outputs.enumerated() {
            print("🔍 输出 #\(i+1) (前100字符): \(output.prefix(100))")
        }
        if inputs.count > 0 {
...
extension ProblemParser {
    /// 检测题目是否包含多组测试用例
    private static func detectMultipleTestCases(from inputSpec: [ContentElement]) -> Bool {
        // 提取所有文本内容
    ...
    private static func extractTextFromElements(_ elements: [ContentElement]) -> String {
        var result = ""
    ...
    private static func adjustSamplesForMultipleTestCases(_ samples: [TestSample]) -> [TestSample] {
